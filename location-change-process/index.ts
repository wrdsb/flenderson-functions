...
const locationChangeProcess: AzureFunction = async function (context: Context, triggerMessage: any): Promise<void> {
    const functionInvocationID = context.executionContext.invocationId;
    const functionInvocationTime = new Date();
    const functionInvocationTimestamp = functionInvocationTime.toJSON();  // format: 2012-04-23T18:25:43.511Z

    const functionName = context.executionContext.functionName;
    const functionEventType = 'WRDSB.Flenderson.Location.Change.Process';
    const functionEventID = `flenderson-functions-${functionName}-${functionInvocationID}`;
    const functionLogID = `${functionInvocationTime.getTime()}-${functionInvocationID}`;

    const logStorageAccount = process.env['storageAccount'];
    const logStorageKey = process.env['storageKey'];
    const logStorageContainer = 'function-location-change-process-logs';
    ...
    switch (operation) {
        case 'create':
            ...
            events = events.concat(craftLocationCreateEvent(oldRecord, newRecord));
        ...
        case 'delete':
            ...
            events = events.concat(craftLocationDeleteEvent(oldRecord, newRecord));
        ...
        case 'update':
            ...
            events = events.concat(craftLocationUpdateEvent(oldRecord, newRecord));
...
export default locationChangeProcess;
