...
const groupPersist: AzureFunction = async function (context: Context, triggerMessage: any): Promise<void> {
    const functionInvocationID = context.executionContext.invocationId;
    const functionInvocationTime = new Date();
    const functionInvocationTimestamp = functionInvocationTime.toJSON();  // format: 2012-04-23T18:25:43.511Z

    const functionName = context.executionContext.functionName;
    const functionEventType = 'WRDSB.Flenderson.HRIS.Group.Persist';
    const functionEventID = `flenderson-functions-${functionName}-${functionInvocationID}`;
    const functionLogID = `${functionInvocationTime.getTime()}-${functionInvocationID}`;

    const logStorageAccount = process.env['storageAccount'];
    const logStorageKey = process.env['storageKey'];
    const logStorageContainer = 'function-group-persist-logs';
    ...
    switch (operation) {
        case 'delete':
            ...
            eventType = 'Flenderson.Group.Delete';
            break;
        case 'patch':
            newRecord = doPatch(oldRecord, payload);
            eventType = 'Flenderson.Group.Patch';
            break;
        case 'replace':
            newRecord = doReplace(oldRecord, payload);
            eventType = 'Flenderson.Group.Replace';
            break;
        default:
            break;
    }
